...
os.makedirs(f"{RESULTADOS_DIR}/Medidas_estructurales", exist_ok=True)
	...
	for red in configuracion.RED:
		...
		for ruteo in configuracion.ROUTING:
			...
			for long_enlace in configuracion.LONG_ENLACES:
				...
				primero=open(f"{configuracion.RESULTADOS_DIR}/{nombre_red}/R{r}/{routing}/D{long_enlace}/datos-promedio.csv","r")
				...
				grados=open(f"{RESULTADOS_DIR}/{nombre_red}/R{r}/{routing}/D{long_enlace}/datos-promedio_grados.csv","r")
			...
			datosPromedio=open(f"{RESULTADOS_DIR}/Medidas_estructurales/Medidas_estructurales_{nombre_red}_R{r}_{routing}.csv","w")
			cols_enlace=""
			for long_enlace in configuracion.LONG_ENLACES:
			...
			for lista in LIST_GRAGEN:
				if len(lista)>maximo:
					maximo=len(lista)
			if configuracion.RED=="anillo":
				nodos=configuracion.NODOS_ANILLO
			else:
				nodos=configuracion.ROWS*configuracion.COLUMNS
