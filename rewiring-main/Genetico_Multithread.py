...
def tam(a, b, precision):
    longitud = b-a
    pedazos = longitud*10**precision
    i=0
    while pedazos > 2**i:
        i = i+1
    return i

def binario_entero(a):
    n = len(a)
    decimal = sum([a[i]*2**(n-1-i) for i in range(n)])
    return decimal

def entero_flotante(decimal, a = 0, b = 1, prec = 4):
    n = tam(a , b, prec)
...
def genetico(configuracion, corrida, n_generaciones, n_individuos, longitud, degradacion, pool_hilos, n_torneo, n_padres, n_cruza, pc, pm_binaria):
    ...
    for k in range(1, n_generaciones + 1):
        ...
        while len(poblacion_nueva) < n_individuos:
            ...
            padre_1.ALPHA = Real_A_Binario(padre_1.ALPHA, 6)
            padre_2.ALPHA = Real_A_Binario(padre_2.ALPHA, 6)
        ...
        for hijo in poblacion_nueva:
            ...
            recuperando_alpha = Binario_A_Real(hijo.ALPHA, 6)
        ...
        #print(f'nueva_generacion: {len(nueva_generacion)}')
        #--------- CREAR LOS EXPERIMENTOS PARA LOS INVIVIDUOS DE LA NUEVA POBLACION ----------------#
        #------( Solo lanzar los hijos, no los padres que pasan a la siguiente generacion )---------#
