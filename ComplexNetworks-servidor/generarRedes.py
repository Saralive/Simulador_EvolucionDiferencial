...
def ejecutar_experimento(n_individuos, ruta_experimento, ruta_actual, degradacion, ALPHA, VECTOR_POPULARIDAD, VECTOR_DISTANCIA):
    ...
    subprocess.run(['python3', os.path.join(ruta_actual,'0creaCopiaSimulador.py'),
    ...
    subprocess.run(['python3',ruta_formacion_py,
                    str(ruta_experimento),str(ruta_Formacion), str(ruta_Degradacion)],
                    cwd = ruta_Formacion)
    
    subprocess.run(['python3', os.path.join(ruta_actual,'1creaPromediosFormacion.py'),
                    str(ruta_experimento),str(ruta_Formacion), str(ruta_Degradacion)])
    
    #-------(9) EXPERIMENTOS DE DEGRADACION DE REDES ---------------------------------------------------#
    ruta_degradacion_py = os.path.join(ruta_experimento,'degradacion.py')
    subprocess.run(['python3', ruta_degradacion_py,
                    str(ruta_experimento),str(ruta_Formacion), str(ruta_Degradacion)])
    
    subprocess.run(['python3', os.path.join(ruta_actual,'6creaPromediosDegradacion.py'),
                    str(ruta_experimento),str(ruta_Formacion), str(ruta_Degradacion)])
